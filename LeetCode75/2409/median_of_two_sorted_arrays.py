...
def find_median_sorted_arrays(nums1: List[int], nums2: List[int]) -> float:
    ...
    def find_kth_element(i: int, j: int, k: int) -> int:
        ...
        if mid1 < mid2:
            return find_kth_element(
                i + k // 2, j, k - k // 2
            )  # Exclude the first half of the first array
        else:
            return find_kth_element(
                i, j + k // 2, k - k // 2
            )  # Exclude the first half of the second array
...
print(
    f"The median of the two sorted arrays is: {find_median_sorted_arrays(TEST_NUMS1, TEST_NUMS2)}"
)
# Expected output: 2.0
