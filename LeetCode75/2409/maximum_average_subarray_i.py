...
def find_max_average(nums: list[int], k: int) -> float:
    ...
    for i in range(k, len(nums)):   # iterate from k to the end of the list
        s += nums[i] - nums[i - k]  # add the current element and subtract the element k steps back
...
print(f'The maximum average of a subarray of length {TEST_K} is: {find_max_average(TEST_NUMS, TEST_K)}')
